...
class Net(nn.Module):
  def __init__(self):
    ...
    # LAYER 1 sees (32,3,18,84,84)
    self.conv1 = self.conv_layer(3,64,(1,2,2),(1,2,2))
    # LAYER 2 sees (32, 64, 18, 42, 42)
    self.conv2 = self.conv_layer(64,128,(1,2,2),(1,2,2))
    # LAYER 3 sees (32, 128, 18, 21, 21)
    self.conv3 = self.conv_layer(128,256,(2,2,2),(2,2,2))
    # LAYER 4 sees 32, 256, 9, 10, 10
    self.conv4 = self.conv_layer(256,256,(2,2,2),(2,2,2))
    # Convoluted feature map shape (latent space): 32, 256, 4, 5, 5 so after flatten: 25600
